  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 text-white">
        ...
        <h3 className="mt-1 text-2xl font-semibold tracking-tight">{resort.name}</h3>
        <div className="mt-2 flex items-center gap-3 text-xs">
          ...
          {resort.runCount ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-white/15 backdrop-blur px-2.5 py-1">
              <ArrowUp className="size-3.5" /> {resort.runCount} pistes
            </span>
          ) : resort.runKm ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-white/15 backdrop-blur px-2.5 py-1">
              <ArrowUp className="size-3.5" /> {resort.runKm} km pistes
            </span>
          ) : null}
        </div>
      </div>
