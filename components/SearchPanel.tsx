...
export function SearchPanel({ resorts }: { resorts: ResortIndexEntry[] }) {
  ...
  const [filters, setFilters] = useState<Filters>(() => ({
    ...
    minRuns: 0,
    maxRuns: bounds.maxRuns,
    minElevation: 0,
  }));

  const activeCount =
    (filters.countries.size > 0 ? 1 : 0) +
    (filters.minRuns > 0 || filters.maxRuns < bounds.maxRuns ? 1 : 0) +
  ...
  const results = useMemo(() => {
    ...
    return resorts.filter((r) => {
      ...
      if (filters.maxRuns < bounds.maxRuns && runs > filters.maxRuns) return false;
  ...
  }, [resorts, q, filters, bounds.maxRuns]);
              ...
              <section className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm tabular-nums">
                    {filters.minRuns}–{filters.maxRuns}
                  </span>
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        minRuns: Math.min(Number(e.target.value), f.maxRuns),
                      }))
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        maxRuns: Math.max(Number(e.target.value), f.minRuns),
                      }))
                    }
                    className="w-full accent-[color:var(--accent)]"
                  />
